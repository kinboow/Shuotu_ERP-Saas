...
const seedStatements = [
  ...
  `INSERT IGNORE INTO plan_features (plan_id, feature_id, enabled, limit_type, limit_value)
   SELECT p.id, f.id, 1,
     CASE WHEN f.feature_code IN ('platform_authorization', 'enterprise_management', 'member_approval') THEN 'count' ELSE NULL END,
     CASE WHEN p.plan_code = 'free' AND f.feature_code = 'platform_authorization' THEN 1
          WHEN p.plan_code = 'starter' AND f.feature_code = 'platform_authorization' THEN 3
          WHEN p.plan_code = 'pro' AND f.feature_code = 'platform_authorization' THEN 10
          WHEN p.plan_code = 'enterprise' AND f.feature_code = 'platform_authorization' THEN 9999
          WHEN p.plan_code = 'free' AND f.feature_code = 'member_approval' THEN 5
          WHEN p.plan_code = 'starter' AND f.feature_code = 'member_approval' THEN 20
          WHEN p.plan_code = 'pro' AND f.feature_code = 'member_approval' THEN 100
          WHEN p.plan_code = 'enterprise' AND f.feature_code = 'member_approval' THEN 9999
          ELSE NULL END
   FROM plans p
   JOIN features f
   WHERE (p.plan_code = 'free' AND f.feature_code IN ('enterprise_management', 'member_approval', 'platform_authorization'))
      OR (p.plan_code = 'starter' AND f.feature_code IN ('enterprise_management', 'member_approval', 'platform_authorization', 'order_management', 'inventory_management'))
      OR (p.plan_code = 'pro' AND f.feature_code IN ('enterprise_management', 'member_approval', 'platform_authorization', 'order_management', 'inventory_management', 'finance_management', 'remote_print', 'courier_reports'))
      OR (p.plan_code = 'enterprise')`
];
...
async function ensurePlatformAdminSchema() {
  ...
  const bootstrapUsername = process.env.PLATFORM_ADMIN_BOOTSTRAP_USERNAME || 'platform_admin';
  const bootstrapPassword = process.env.PLATFORM_ADMIN_BOOTSTRAP_PASSWORD || 'admin123';
  ...
  if (Number(existingUsers[0]?.count || 0) === 0) {
    ...
    await sequelize.query(
      `INSERT INTO platform_users (username, password_hash, real_name, role_code, status)
       VALUES (:username, :passwordHash, :realName, 'platform_super_admin', 'ACTIVE')`,
      {
        replacements: {
...
module.exports = { ensurePlatformAdminSchema };
