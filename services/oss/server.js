...
const CATEGORIES = ['pdf', 'images', 'temp', 'documents', 'permanent'];
const PERMANENT_CATEGORIES = ['permanent']; // 永久文件分类，无需签名即可访问
...
app.post('/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    let fileUrl, expiresAt;
    
    // 永久文件分类返回不带签名的URL
    if (PERMANENT_CATEGORIES.includes(category)) {
      fileUrl = `${baseUrl}/file/${category}/${fileId}${ext}`;
      expiresAt = null; // 永不过期
    } else {
      // 生成签名URL
      ...
      fileUrl = `${baseUrl}/file/${category}/${fileId}${ext}?expires=${expires}&signature=${signature}`;
    ...
    res.json({
      ...
      data: {
        ...
        signedUrl: fileUrl,
        url: fileUrl,
        permanent: PERMANENT_CATEGORIES.includes(category),
        expiresAt,
...
app.get('/file/:category/:fileName', async (req, res) => {
  try {
    ...
    // 永久文件分类无需签名验证
    const isPermanent = PERMANENT_CATEGORIES.includes(category);
...
app.get('/proxy/image', async (req, res) => {
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ success: false, message: '图片获取失败' });
    }
    ...
    res.status(500).json({ success: false, message: '图片代理失败: ' + error.message });
  }
});

/**
 * 图片代理接口 - 直接返回图片流
 * GET /proxy/image/stream?url=xxx
 */
app.get('/proxy/image/stream', async (req, res) => {
  try {
    const { url } = req.query;
    if (!url) {
      return res.status(400).json({ success: false, message: '缺少url参数' });
    ...
    if (!response.ok) {
      return res.status(response.status).send('图片获取失败');
    }
